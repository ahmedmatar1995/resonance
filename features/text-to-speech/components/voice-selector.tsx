...
import { VOICE_CATEGORY_LABELS } from "@/constants";
...
export function VoiceSelector() {
  ...
  const { systemVoices, customVoices, allVoices: voices } = useTTSVoices();
      ...
      >
        <SelectTrigger className="w-full h-auto gap-1 rounded-lg bg-white px-2 py-1">
          <SelectValue>
            {currentVoice && (
              <>
                <VoiceAvatar seed={voiceId} name={currentVoice.name} />
            ...
            <SelectGroup>
              ...
              {customVoices.map((v) => (
                <SelectItem value={v.id} key={v.id}>
                  ...
                  <span className="truncate text-sm font-medium tracking-tight">
                    {v.name}
                    {v.category && ` - ${VOICE_CATEGORY_LABELS[v.category]}`}
                  </span>
                </SelectItem>
              ))}
            </SelectGroup>
